 ...
 *  mode. Additionally, for this instance of  `DJIMediaFile`, the
...
/**
 *  Determines whether the lens focus assistant is enabled.  When the focus
 *  assistant is enabled,  the camera live view will digitally zoom into the focus
 *  target area so the user can see the quality of focus. Focus assistant can be
 *  used for both manual focus (MF) and auto focus (AF). It is available only when
 *  `isAdjustableFocalPointSupported`  returns `YES`. Supported only by the X5, X5R,
